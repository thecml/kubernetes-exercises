%=====================================
% Author: Christian Fischer Pedersen
% About: Slide template
%=====================================
\input{cfp_slide_style.tex}
\usepackage{listings}
%\setbeameroption{show notes} un-comment to see the notes

%===============Title slide
\slidetitlepage{Cloud Computing, Part 1}{Distributed Systems, 3rd Semester, BSc}

\begin{document}
\begin{frame}[plain]
\titlepage
\end{frame}

%===============Outline slide: Parts
\begin{frame}
\frametitle{Outline}
\tableofcontents
\end{frame}

%===============Slides
\section[Cloud computing]{Introduction}
\frame{
	\frametitle{What is cloud computing?}
	\framesubtitle{}
	The cloud
	\begin{itemize}
		\item A metaphor for the Internet. Something that is present remotely.
	\end{itemize}
	Cloud computing
	\begin{itemize}
		\item The delivery of online computing services
		\item Most often, these include servers, storage, databases\\
		software and analytics
		\item Management is done by third-party
		\item Services are often at an enterprise-level
	\end{itemize}
	Benefits
	\begin{itemize}
		\item Lower costs
		\item Accessibility
		\item Productivity
		\item Scalability
		\item Updates
	\end{itemize}	
}

\section[Cloud computing]{Background}
\frame{
	\frametitle{Cloud computing: Background}
	\framesubtitle{}
	Origin
	\begin{itemize}
		\item Around 1970, the concept of virtual machine's (VMs) was invented
		\item In 1999, Salesforce.com delivered applications to users using
		a simple website
		\item In 2002, Amazon provided the first cloud service
		\item In 2009, Google Apps saw the light of day
		\item In 2009, Microsoft launched Azure
	\end{itemize}
	Most known providers
	\begin{itemize}
		\item Google Cloud (Search, Gmail)
		\item Amazon's AWS (Online marketplace)
		\item Microsoft Azure (OneDrive)
	\end{itemize}
	Deployment models
	\begin{itemize}
		\item Private cloud
		\item Public cloud (e.g. AWS, Google)
		\item Hybrid cloud
	\end{itemize}	
}

\section[Microservices]{Microservices}
\frame{
	\frametitle{Monolithic vs. microservices}
	\framesubtitle{}
	Monolithic
	\begin{itemize}
		\item Everything in one place
		\item Tightly coupled, runs as a single service
		\item Developed and scaled as one
		\item Hard to maintain
	\end{itemize}
	SoA-architecture
	\begin{itemize}
		\item Service-oriented architecture (SOA)
		\item Splits software application in smaller units
		\item Units communicate over network, but functions separately 
	\end{itemize}
	What microservices are
	\begin{itemize}
		\item Modern version of the SoA-style
		\item Small, independent services that do one thing
		\item Highly maintainable, testable, loosely coupled
		\item An analogy: "Instead of having one machine build\\
		a whole car, get multiple factories to work at the same time.
	\end{itemize}	
}

\frame{
	\frametitle{SoA}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{figures/1_1.png}
		\caption{Fig. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\frame{
	\frametitle{SoA}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{figures/1_2.png}
		\caption{Fig. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\frame{
	\frametitle{SoA}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{figures/1_3.png}
		\caption{Fig. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\section[Containers]{Linux containers}
\frame{
	\frametitle{Linux containers}
	\framesubtitle{}
	Motivation
	\begin{itemize}
		\item We can't give every software component its own VM
		\item VM's are manual. We need automation
		\item Changes once place should not impact others
	\end{itemize}
	What Linux containers are
	\begin{itemize}
		\item Containers are isolated software environments
		\item Application and dependencies bundled inside
		\item A lightweight version of VM's
	\end{itemize}
}

\frame{
	\frametitle{A normal VM vs containers}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{figures/1_4.png}
		\caption{Fig. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\section[Containers]{Docker}
\frame{
	\frametitle{Docker}
	\framesubtitle{}
	Motivation
	\begin{itemize}
		\item We need suitable tooling to create containers
		\item It should be automated, predictable and fast
		\item It should run everywhere, be modular and scale well
	\end{itemize}
	What Docker is
	\begin{itemize}
		\item Docker is a container tool that can run and create containers
		\item Containers only see their exact file system
		\item Similar to VM's, but less overhead
		\item Consists of resealable layers
		\item Dockerfile
	\end{itemize}
	Important concepts
	\begin{itemize}
		\item Images. Something you package your application into
		\item Registries. A repository to store your image
		\item Containers. Like normal Linux container.\\Isolated from host and other processes
	\end{itemize}
}

\frame{
	\frametitle{The Docker build process}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{figures/1_6.png}
		\caption{Fig. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\frame{
	\frametitle{Running apps in VM's vs. containers}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{figures/1_7.png}
		\caption{Fig. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\section[Kubernetes]{Kubernetes}
\frame{
	\frametitle{Kubernetes}
	\framesubtitle{}
	Motivation
	\begin{itemize}
		\item We need something to manage our containers
		\item Should be reliable, automated and scalable
		\item Should ease the process of deploying containers
	\end{itemize}
	What Kubernetes is
	\begin{itemize}
		\item Open-source container orchestration engine
		\item Made by Google, open-sourced in 2014
		\item Google needed to better utilize their resources
		\item Enables easy deployment, scaling and managing
		\item Exposes whole datacenter as a single platform.
	\end{itemize}
}

\frame{
	\frametitle{Running apps in VM's vs. containers}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{figures/1_8.png}
		\caption{Fig. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\section[Kubernetes]{Kubernetes cont.}
\frame{
	\frametitle{Kubernetes cont.}
	\framesubtitle{}
	Main features of Kubernetes
	\begin{itemize}
		\item Keep containers running.
		\item Scaling copies.
		\item Hitting a moving target.
		\item Master and nodes.
	\end{itemize}
	Benefits of using Kubernetes
	\begin{itemize}
		\item Simplifying application deployment
		\item Achieve better utilization
		\item Health checking and self-healing
		\item Automatic scaling
		\item Access to services via API/DNS
	\end{itemize}
}

\section[Kubernetes]{Kubernetes cont.}
\frame{
	\frametitle{Kubernetes cont.}
	\framesubtitle{}
	The master node hosts the Control Plane and worker nodes run deployments.
	The master
	\begin{itemize}
		\item API Server, which you and Control Plane components communicate with
		\item The Scheduler, which schedules your apps
		\item The Controller Manager, keeps track of workers among others
		\item etcd, a distributed db that stores cluster configuration.
	\end{itemize}
	The nodes
	\begin{itemize}
		\item Docker, rtk or another container runtime
		\item Kubelet, which talks to the API server and manages containers
		\item Kube-proxy, which load-balances network traffic
	\end{itemize}
}

\frame{
	\frametitle{The components in Kubernetes}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{figures/1_9.png}
		\caption{Fig. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\frame{
	\frametitle{A basic overview of Kubernetes's architecture}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{figures/1_10.png}
		\caption{Fig. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\section[Kubernetes hands on]{Hands on}
\frame{
	\frametitle{Running the busybox image}
	\framesubtitle{}
	Installing Docker and running a Hello World container
	\begin{itemize}
		\item Busybox is a single executable with many UNIX tools.
	\end{itemize}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{listings/2_1.png}
		\caption{Listing. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
	\begin{lstlisting}[numbers=none, basicstyle=\ttfamily]
		$ docker run busybox echo "Hello world"
		$ docker run <image>
		$ docker run <image>:<tag>
	\end{lstlisting}
}

\frame{
	\frametitle{Running echo "Hello world" in a container}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{figures/2_1.png}
		\caption{Fig. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\frame{
	\frametitle{Creating a Node.js app}
	\framesubtitle{}
	\begin{itemize}
		\item We make a simple HTTP app that can receive\\
		and reply to requests with its hostname
		\item Using Node.js and JavaScript
	\end{itemize}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{listings/2_2.png}
		\caption{Listing. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\frame{
	\frametitle{Creating a Dockerfile for the image}
	\framesubtitle{}
	\begin{itemize}
		\item We need a Dockerfile to create an image
		\item It describes the app and its dependencies
	\end{itemize}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{listings/2_3.png}
		\caption{Listing. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\frame{
	\frametitle{Building the container image}
	\framesubtitle{}
	\begin{itemize}
		\item Docker daemon builds the image.
	\end{itemize}
	\begin{lstlisting}[numbers=none, basicstyle=\ttfamily]
		$ docker build -t kubia .
	\end{lstlisting}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{listings/2_4.png}
		\caption{Listing. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{listings/2_5.png}
		\caption{Listing. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\frame{
	\frametitle{Building a new container image from Dockerfile}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{figures/2_2.png}
		\caption{Fig. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\frame{
	\frametitle{The layers of a container image}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{figures/2_3.png}
		\caption{Fig. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\frame{
	\frametitle{More container commands}
	\framesubtitle{}
	Running the container image
	\begin{lstlisting}[numbers=none, basicstyle=\ttfamily]
		$ docker run --name kubia-container -p 8080:8080 -d kubia
		$ curl localhost:8080
	\end{lstlisting}
	Exploring the inside of a running container
	\begin{lstlisting}[numbers=none, basicstyle=\ttfamily]
		$ docker exec -it kubia-container bash
		$ ps aux
	\end{lstlisting}
	Stopping and removing a container
	\begin{lstlisting}[numbers=none, basicstyle=\ttfamily]
		$ docker stop kubia-container
		$ docker rm kubia-container
	\end{lstlisting}
}

\frame{
	\frametitle{Output of container commands}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{listings/2_6.png}
		\caption{Listing. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{listings/2_7.png}
		\caption{Listing. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{listings/2_8.png}
		\caption{Listing. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\frame{
	\frametitle{Pushing the image to the registry}
	\framesubtitle{}
	\begin{lstlisting}[numbers=none, basicstyle=\ttfamily]
		$ docker tag kubia <dockerid>/kubia
	\end{lstlisting}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{listings/2_9.png}
		\caption{Listing. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
	\begin{lstlisting}[numbers=none, basicstyle=\ttfamily]
		$ docker push <dockerid>/kubia
		$ docker run -p 8080:8080 -d <dockerid>/kubia
	\end{lstlisting}
}

\frame{
	\frametitle{Setting up a Kubernetes cluster with Minikube}
	\framesubtitle{}
	What is minikube
	\begin{itemize}
		\item minikube implements a local Kubernetes cluster
		\item Runs on macOS, Linux and Windows
		\item Used for Kubernetes development and prototyping
		\item Exercises and project make use of minikube
	\end{itemize}
	You can also order a hosted Kubernetes cluster at \\
	Google, Amazon, Microsoft Azure etc.
	\begin{lstlisting}[numbers=none, basicstyle=\ttfamily]
		$ minikube start
		$ kubectl get nodes
		$ kubectl describe node gke-kubia-85f6-node-0rrx
	\end{lstlisting}
}

\frame{
	\frametitle{Introducing pods}
	\framesubtitle{}
	Before we deploy our app, we need to know what pod is
	\begin{itemize}
		\item One of main building blocks in Kubernetes
		\item A pod is a group of one or more tightly\\
		 related containers that run together and share\\
		 the same Linux namespace
		 \item Each pod is like a separate logical machine.
		 \item All containers in a pod will appear to be\\
		 running on the same logical machine.
	\end{itemize}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{figures/2_5.png}
		\caption{Fig. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\frame{
	\frametitle{Pod commands}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{listings/2_14.png}
		\caption{Listing. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{listings/2_15.png}
		\caption{Listing. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\frame{
	\frametitle{Running our first app on Kubernetes}
	\framesubtitle{}
	\begin{lstlisting}[numbers=none, basicstyle=\ttfamily]
		$ kubectl run kubia --image=<dockerid>/kubia --port=8080 --generator=run-pod/v1
		pod/kubia created
	\end{lstlisting}
}

\frame{
	\frametitle{Accessing our application}
	\framesubtitle{}
	We create a service that exposes our pod to us
	\begin{lstlisting}[numbers=none, basicstyle=\ttfamily]
	$ kubectl expose pod kubia --type=NodePort --name kubia-http
	\end{lstlisting}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{listings/2_16.png}
		\caption{Listing. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{listings/2_17.png}
		\caption{Listing. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\frame{
	\frametitle{Accessing our application cont.}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{figures/2_7.png}
		\caption{Fig. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

\frame{
	\frametitle{Accessing our application cont.}
	\framesubtitle{}
	\begin{figure}[htbp!]
		\includegraphics[width=0.7\textwidth]{figures/2_8.png}
		\caption{Fig. by courtesy of Marko Luksa}
		\label{fig:}
	\end{figure}
}

%===============References
\begin{frame}[allowframebreaks]
\frametitle{References}
%\def\section*#1{}%remove auto heading for references
\fontsize{5pt}{5pt}\selectfont
\def\newblock{\hskip .11em plus .33em minus .07em}
\bibliographystyle{chicago}
\bibliography{cl_bibliography}
\normalsize
\end{frame}

\end{document}